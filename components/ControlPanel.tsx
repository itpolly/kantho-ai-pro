...
import { VoiceName, VoiceConfigState, EmotionOption, PacingOption } from '../lib/types.js';
import { VOICE_OPTIONS, PACING_OPTIONS, EMOTION_OPTIONS } from '../lib/constants.js';

interface ControlPanelProps {
  onGenerate: () => void;
  isGenerating: boolean;
  voiceConfig: VoiceConfigState;
  // Fix: changed from (config: VoiceConfigState) => void to support functional updates from useState hook
  onConfigChange: React.Dispatch<React.SetStateAction<VoiceConfigState>>;
  disabled: boolean;
}
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const toggleMode = useCallback(() => {
    // Functional update now correctly types against React.Dispatch<React.SetStateAction<VoiceConfigState>>
    onConfigChange((prevConfig: VoiceConfigState) => ({
      ...prevConfig,
  ...
  const handlePrimaryChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    onConfigChange((prevConfig: VoiceConfigState) => ({ ...prevConfig, primaryVoice: e.target.value as VoiceName }));
  }, [onConfigChange]);

  const handleSecondaryChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    onConfigChange((prevConfig: VoiceConfigState) => ({ ...prevConfig, secondaryVoice: e.target.value as VoiceName }));
  }, [onConfigChange]);
  ...
  const handlePacingSliderChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (option) {
      onConfigChange((prevConfig: VoiceConfigState) => ({ ...prevConfig, pacing: option.value }));
    }
  }, [onConfigChange]);

  const handlePacingButtonClick = useCallback((pacingValue: PacingOption) => {
    onConfigChange((prevConfig: VoiceConfigState) => ({ ...prevConfig, pacing: pacingValue }));
  }, [onConfigChange]);

  const handleEmotionSelect = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    onConfigChange((prevConfig: VoiceConfigState) => ({ ...prevConfig, emotion: e.target.value as EmotionOption }));
  }, [onConfigChange]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-8 flex flex-col gap-6 animate-fade-in-up">
        ...
        <div className="flex flex-col lg:flex-row gap-6 pb-6 border-b border-slate-800/50">
          <div className="w-full lg:w-1/3">
             ...
             <button
              onClick={toggleMode}
              disabled={disabled}
              className={`relative w-full p-1 rounded-xl flex items-center bg-slate-950 border border-slate-800 transition-all ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
            ...
            >
              <div 
                 className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-slate-800 rounded-lg shadow-sm transition-all duration-300 ease-out border border-slate-700 ${voiceConfig.mode === 'single' ? 'left-1' : 'left-[calc(50%+0px)] translate-x-0'}`} 
                 aria-hidden="true"
              />
              ...
              <div className="relative">
                ...
                >
                  {VOICE_OPTIONS.map((voice) => (
                    <option key={`p-${voice.value}`} value={voice.value}>{voice.label}</option>
                  ))}
                </select>
            ...
            {voiceConfig.mode === 'multi' && (
              <div className="animate-fade-in">
                ...
                <div className="relative">
                  ...
                  >
                    {VOICE_OPTIONS.map((voice) => (
                      <option key={`s-${voice.value}`} value={voice.value}>{voice.label}</option>
                    ))}
                  </select>
                  <Mic className="absolute right-4 top-3.5 text-slate-600 pointer-events-none" size={16} aria-hidden="true" />
                </div>
                ...
                <div className="flex justify-between mt-2 text-[10px] font-medium uppercase tracking-widest px-1 relative z-10">
                   {PACING_OPTIONS.map((opt, index) => (
                      <button 
                        ...
                        disabled={disabled}
                        className={`focus:outline-none transition-colors ${voiceConfig.pacing === opt.value ? 'text-teal-400 font-bold scale-110' : 'text-slate-500 hover:text-slate-300'}`}
                      >
                        {pacingLabels[index]}
                      </button>
                   ))}
      ...
      <button
        ...
        className={`group w-full flex items-center justify-center space-x-3 py-6 rounded-2xl font-bold text-lg tracking-wide transition-all duration-300 transform shadow-2xl relative overflow-hidden ${disabled || isGenerating ? 'bg-slate-900 text-slate-500 cursor-not-allowed border border-slate-800' : 'bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 text-white hover:shadow-[0_0_40px_rgba(245,158,11,0.4)] hover:scale-[1.01]'}`}
      >
        {isGenerating ? (
          <>
